...
Необходимые разрешения добавлены в android часть также, но не теститровались.

# Результаты
## Приложение чата
Целью Study Jam было используя предоставленный API с документацией и шаблоном чата модернизировать 
его выполнив определенный перечень заданий разной сложности.   
В выполненном проекте присутсвует реализация всех заявленных задач.
...
- `intl` - пакет интернациаонализации, тут для преобразования форматов даты и времени
...
## Экран авторизации
Экран авторизации отображает линейный индикатор прогресса, который отображается после нажатия 
на кнопку "Далее", когда в стейте кубита авторизации `AuthCubit` появляется праметр отвечающий за состояние загрузки. После окончания загрузки 
состояние сменяется и соответствующий BlocListener выполняет навигацию на следующую страницу.
...
## Навигация
Навигация построена на базе Navigator 1.0 и именованых маршрутов. При старте приложения
выполяется проверка валидности сохраненного в локальном хранилище токена. Далее осуществляется
...
выполяющего запрос в репозиторий,сменяющего состояния получения геолокации и обрабатывающего 
...
Для компактного размещения изображений изображения отображаются в GridView.

## Отображение вресмени
...
открыть системное окно выбра изображений и после подтверждения выбора получить список объетов
типа XFile. Далее полученный список изображений загружаются в хранилище с помощью реализованной 
фичи `image_upload`.
...
> хранилищ изображений с загрузкой по API. Таковых нашлись, исходя из критерия бесплатности два: 
> https://freeimage.host/ и https://imgbb.com/. Но второй хоть и позволяет загружать и имеет механизм
> удаления загруженной каринки, но имеет на сайте упоминание о платных тарифах. Был выбран первый вариант.
...
использутся единственный параметр - строка со ссылкой на загруженное изображение.
...
обновленный стейт со списком ссылок на изображения. 
В интерфейсе реализован `BlocBuilder`, который при обновлении стейта и появлении ссылок в списке 
послеовательно отображает их в интерфейсе в `GridView`. При нажатии кнопки "Отправить" список ссылок 
добавляется в параметр `images` при отправке. После отправки список изображений очищается.

## Кнопка отправить
При нажатии кнопки отправить происходит проверка, какие дынные для отправки были подготовлены и 
в зависимости от наличия локации или изображения или при отстутсвии и того и другого отправляет 
соответсвеющий тип сообщения.

## Аватарки
Цвет аватарки получается путем бинарной оперции или над шестнадцатиричным значением черного
цвета и хешкода строки с именем. Ввиду того, что у пользователей могло не быть имени, состоящего
из двух слов, алгоритм формирования текста внутри аватара необходимо было изменить.

## Баблы
Для баблов используется библиотека bubble, настроены отступы и выступ сверху.
...
дополнительно реализованного метода репозитороия `IChatRepository` - `getLocalUser()`,
которая выполняет метод `getUser()` без параметра из API и получает объект локального 
пользователя из ответа.   
Имена пользователей синхронизируюся с github и при разовом сохранении его в локальном хранилище 
...
Также реализована возможность загружать изображение и устанавливать его в качетсве аватара 
темы. Загрузка изображения осуществляется с помощью фичи `image_upload` описанной ранее. 
После загрузки цветной круг с текстом заменятеся загруженным изображением.
При создании темы ссылка на загруженное изображение отправлятеся в параметре `avatar`
модели `ChatTopicSendDto` наряду с полученными из тектовых полей параметров `name` и `description`.
...
Для получения сообщений из опреденной темы и для отпавки сообщения в опреденной тему во все методы
...
момент туда успели попасть констаты названий путей роутинга, адреса, пути и параметры запроса
к API загрузки изображений и наблюдатель состояний блока/кубита.

## TODO (пока доступно api)
[ ] Ввиду ограниченного времени пришлось делать выбор - реализовывать задачи или реализовать архитекруру. 
В итоге фичи `chat` и `topics` не реализованы полноценно с точки зренения стейт
менеджмента. В них заложено использование `cubit`, но времени на полноценную реализацию функций и 
стейтов не хватило. Основное, что хотелось успеть добавить - стейт загрузки, получения данных и ошибки для
отображения, для демонстрации индикатора загрузки и отображения сообщения об ошибке, приходящего,
например, в случае когда в топике нет сообщений.
[ ] Отправка и отображение комбинрированного сообщения с геолокацией и изображением, добавить для этого модель 
и соответсвующие методы в репозиторий. 
[ ] Сохранение позиции скролла при переходе в конкретный чат с экрана тем
[ ] Дизайн. Дизнайн не был приоритетным. Как я сейчас вижу настолько, что ни у каких типов сообщений, 
кроме обычного не реализована проверка `isUserLocal` для отображения цвета своего сообщения =)
